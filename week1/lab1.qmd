...
penguins_row <- penguins %>%
  count(species, island)
  group_by(species) %>% 
  mutate(prop = n / sum(n))
  
penguins_col <- penguins %>%
  count(species, island)
  group_by(island) %>% 
  mutate(prop = n / sum(n))
...
Histograms represent frequency of a given binned value along a continuous x axis, bar plots are for discrete values and are not binned.
...
win1sd <- mean(body_mass >= mean_mass - sd_mass &
  body_mass <= mean_mass + sd_mass)

win2sd <- mean(body_mass >= mean_mass - 2*sd_mass &
  body_mass <= mean_mass + 2*sd_mass)
...
```{r}
bees <- read_csv(here('week1', 'Species Occurrence Records for MD and DC_Data.csv')) %>% 
  clean_names() %>% 
  select(name, sex, determined_when, state, county, city, site)
glimpse(bees)
...
time_counts <- bees %>% 
  group_by(determined_when) %>% 
  count()
```

Try to make a contigency table
```{r}
table <- bees %>% 
  drop_na(state, sex) %>% 
  filter(sex != 'Null',
